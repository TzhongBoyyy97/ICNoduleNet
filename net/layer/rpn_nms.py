...
def make_rpn_windows(f, cfg, mode):
    """
...
#             windows.append([z, y , x , np.random.normal(a[0], scale=1.0, size=None), np.random.normal(a[1], scale=1.0, size=None), np.random.normal(a[2], scale=1.0, size=None)])
#     else:
    for z, y , x , a in itertools.product(oz, oh , ow , anchors):
